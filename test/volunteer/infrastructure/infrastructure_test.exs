defmodule Volunteer.InfrastructureTest do
  ...
  describe "get_region_config/2" do
    test "returns correctly when data exists" do
      assert {:ok, ["cfo-announcements@iicanada.net"]} == Infrastructure.get_region_config(2, :marketing_request_email)
    end

    test "returns fallback when region doesn't exist" do
      assert {:ok, []} == Infrastructure.get_region_config(Faker.UUID.v4(), :marketing_request_email)
    end
