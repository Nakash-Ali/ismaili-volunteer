...
export default class extends Controller {
	connect() {
		...
		this.inputIsInvalidAlready = this.$input.hasClass(
			this.data.get('inputInvalidClass'),
		)
	...
	check() {
		...
		if (delta > 0) {
			window.requestAnimationFrame(() =>
				this.$this.text(`${delta} ${word} remaining`),
			)
		} else if (delta === 0) {
			window.requestAnimationFrame(() =>
				this.$this.text(`no ${word} remaining`),
			)
		} else if (delta < 0) {
			window.requestAnimationFrame(() =>
				this.$this.text(`${Math.abs(delta)} ${word} extra`),
			)
		} else {
		...
		if (this.prevCharCount === currCharCount) {
			...
			if (this.inputIsInvalidAlready === false) {
				window.requestAnimationFrame(() => {
					this.$this
						.addClass(this.data.get('thisInvalidClass'))
						.removeClass(this.data.get('thisValidClass'))
			...
			if (this.prevCharCount <= this.data.get('max')) {
				...
				this.prevCharCount > this.data.get('max') &&
				this.inputIsInvalidAlready === false
			) {
				window.requestAnimationFrame(() => {
					this.$this
						.addClass(this.data.get('thisValidClass'))
						.removeClass(this.data.get('thisInvalidClass'))
